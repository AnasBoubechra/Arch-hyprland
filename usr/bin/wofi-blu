...
device_menu() {
    ...
    chosen="$(echo -e "$options" | run_wofi "$device_name")"
...
show_menu() {
    ...
    # Open wofi menu, read chosen option
    chosen="$(echo -e "$options" | run_wofi "Bluetooth")"
...
# wofi command to pipe into. Extra arguments to wofi-bluetooth are passed through to wofi. This
# allows the user to set fonts, sizes, colours, etc.
run_wofi() {
    wofi -n -d "${original_args[@]}" -p "$1"
}
