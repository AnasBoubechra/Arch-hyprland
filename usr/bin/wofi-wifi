...
# If there are more than 22 SSIDs, the menu will still only have 22 lines
if [ "$LINENUM" -gt 22 ] && [[ "$CONSTATE" =~ "enabled" ]]; then
	LINENUM=22
...
CHENTRY=$(echo -e "$TOGGLE\nmanual\n$LIST" | uniq -u | \
    wofi -i --dmenu -p "Wi-Fi SSID: " --width "$WWIDTH" --lines ${LINENUM} --cache-file /dev/null | awk -F "[  ]{2,}" '{gsub(/<[^>]*>/, ""); print $0}')
